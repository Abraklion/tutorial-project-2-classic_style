...
const burger = (menuSelector, burgerSelector) => {
   ...
   * МОДУЛЬ БУРГЕР МЕНЮ
   *
   * menuSelector    -> селектор меню
   * burgerSelector  -> селектор который открывает и закрывает меню
   */
...
const filter = () => {
  ...
  const typeFilter = markType => {
    /**
     * markType -> NodeList элементов работ
     */
...
const scrolling = upSelector => {
  /**
   * МОДУЛЬ ПЛАВНЫЙ СКРОЛЛ К ЯКАРЮ
   *
   * upSelector  -> серектор кнопка вверх
   */
